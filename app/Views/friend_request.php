...
<table class="user-table">
  ...
  <?php if (!empty($users)): ?> 
    <?php foreach ($users as $user) { ?>
      <tr>
        <td class="username"><?= $user->username1 ?></td>
        <td class="actions">
          <?php if ($user->status === 'friends') { ?>
            <form action="/user/chat/<?= $user->username1 ?>" method="get">
              <input type="hidden" name="start_chat" value="<?= $user->username1 ?>">
              <button class="chat-button" type="submit">Chat</button>
            </form>
          <?php } else if ($user->status === 'pending') { ?>
            <div class="friend-request">
              <form action="/user/friend-request/accept" method="post">
                <input type="hidden" name="username" value="<?= $user->username1 ?>">
                <button class="accept-button" type="submit">Accept</button>
              </form>
              <form action="/user/friend-request/delete" method="post">
                <input type="hidden" name="username" value="<?= $user->username1 ?>">
